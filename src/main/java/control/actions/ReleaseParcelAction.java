...
public class ReleaseParcelAction extends ActionSupport implements SessionAware,
	...
	public String searchParcelAction() throws Exception {
		...
		List<Parcel> list = pm.searchRealease(param, user.getUserDetail()
				.getStation().getID());
		...
		for (Parcel parcels : list) {
			String temp = "<tr><td>"
					+ parcels.getID()
					+ "</td><td>"
					+ parcels.getStarts().getName()
					+ "</td><td>"
					+ parcels.getSender().getNic()
					+ "</td><td>"
					+ parcels.getSender().getName()
					+ "</td><td>"
					+ parcels.getReceiver().getNic()
					+ "</td><td>"
					+ parcels.getReceiver().getName()
					+ "</td><td>"
					+ "<td><button type='button' class='btn btn-default btn-s release' name='"
					+ parcels.getID()
					+ "'><span class='glyphicon glyphicon-ok'></span> Release</button></td></tr>";
			sb.append(temp);
		}
