	...
	public String addDevice() throws Exception {
		...
		if (user != null && user.getUserPrivilege().isAdd_device()) {
			...
			for (Device device : list) {
				String temp = "<tr><td>" + device.getIMEI() + "</td><td>"
						+ device.getAssigned().getUserName() + "</td></tr>";
				sb.append(temp);
			}
		...
		if (user != null && user.getUserPrivilege().isRemove_device()) {
			...
			for (Device device : list) {
				String temp = "<tr><td>"
						...
						+ "</td><td><button type='button' class='btn btn-default btn-s change_di' name='"
						+ device.getID()
						+ "'><span class='glyphicon glyphicon-pencil'></span> Change</button></td><td><button type='button' class='btn btn-default btn-s deletes_di' name='"
						+ device.getID()
