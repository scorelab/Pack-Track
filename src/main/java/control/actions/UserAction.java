		...
		if (user!=null && user.getUserPrivilege().isRemove_user()) {
			...
			for (User users : list) {
				String temp = "<tr><td>"
						...
						+ "</td><td>"
						+ users.getNicNumber()
						+ "</td><td><button type='button' class='btn btn-default btn-lg change' name='"+users.getUserName()+"'><span class='glyphicon glyphicon-pencil'></span> Change</button></td><td><button type='button' class='btn btn-default btn-lg'><span class='glyphicon glyphicon-trash'></span> Delete</button></td></tr>";
				sb.append(temp);
			}
			tableRows = sb.toString();
			return SUCCESS;
