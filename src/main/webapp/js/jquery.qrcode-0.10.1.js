...
(function() {
	...
	var QRCode = function(text, level, version, quiet) {
		...
		// `qrcode` is the single public function that will be defined by the
		// `QR Code Generator`
		...
		var qrModuleCount = qr.getModuleCount(), quietModuleCount = qr
				.getModuleCount()
				+ 2 * quiet, isDark = function(row, col) {

			row -= quiet;
			col -= quiet;

			if (row < 0 || row >= qrModuleCount || col < 0
					|| col >= qrModuleCount) {
				return false;
			}
			...
			this.isDark = function(row, col) {
				...
				var ml = col * moduleSize, mt = row * moduleSize, mr = ml
						+ moduleSize, mb = mt + moduleSize;

				return prevIsDark(row, col)
						&& (l > mr || ml > r || t > mb || mt > b);
			};
		};
	...
	drawBackgroundLabel = function(qr, context, settings) {
		...
		var size = settings.size, font = "bold " + (settings.mSize * size)
				+ "px " + settings.fontname, ctx = $('<canvas/>')[0]
				.getContext("2d");

		ctx.font = font;

		var w = ctx.measureText(settings.label).width, sh = settings.mSize, sw = w
				/ size, sl = (1 - sw) * settings.mPosX, st = (1 - sh)
				* settings.mPosY, sr = sl + sw, sb = st + sh, pad = 0.01;
		...
		context.font = font;
		context.fillText(settings.label, sl * size, st * size + 0.75
				* settings.mSize * size);
	},

	drawBackgroundImage = function(qr, context, settings) {

		var size = settings.size, w = settings.image.naturalWidth || 1, h = settings.image.naturalHeight || 1, sh = settings.mSize, sw = sh
				* w / h, sl = (1 - sw) * settings.mPosX, st = (1 - sh)
				* settings.mPosY, sr = sl + sw, sb = st + sh, pad = 0.01;
		...
		context.drawImage(settings.image, sl * size, st * size, sw * size, sh
				* size);
	},
	...
	drawBackground = function(qr, context, settings) {
		...
		if ($(settings.background).is('img')) {
			context.drawImage(settings.background, 0, 0, settings.size,
					settings.size);
		} else if (settings.background) {
			context.fillStyle = settings.background;
			context.fillRect(settings.left, settings.top, settings.size,
					settings.size);
		}
	...
	drawModuleDefault = function(qr, context, settings, left, top, width, row,
			col) {
	...
	drawModuleRounded = function(qr, context, settings, left, top, width, row,
			col) {

		var isDark = qr.isDark, right = left + width, bottom = top + width, radius = settings.radius
				* width, rowT = row - 1, rowB = row + 1, colL = col - 1, colR = col + 1, center = isDark(
				row, col), northwest = isDark(rowT, colL), north = isDark(rowT,
				col), northeast = isDark(rowT, colR), east = isDark(row, colR), southeast = isDark(
				rowB, colR), south = isDark(rowB, col), southwest = isDark(
				rowB, colL), west = isDark(row, colL);

		if (center) {
			drawModuleRoundedDark(context, left, top, right, bottom, radius,
					!north && !west, !north && !east, !south && !east, !south
							&& !west);
		} else {
			drawModuleRoundendLight(context, left, top, right, bottom, radius,
	...
	drawModules = function(qr, context, settings) {
		...
		var moduleCount = qr.moduleCount, moduleSize = settings.size
				/ moduleCount, fn = drawModuleDefault, row, col;
		...
		for (row = 0; row < moduleCount; row += 1) {
			for (col = 0; col < moduleCount; col += 1) {
				...
				var l = settings.left + col * moduleSize, t = settings.top
						+ row * moduleSize, w = moduleSize;
		...
		if ($(settings.fill).is('img')) {
			...
			context.clip();
			context
					.drawImage(settings.fill, 0, 0, settings.size,
							settings.size);
			context.restore();
		} else {
	...
	drawOnCanvas = function(canvas, settings) {
		...
		var qr = createQRCode(settings.text, settings.ecLevel,
				settings.minVersion, settings.maxVersion, settings.quiet);
		if (!qr) {
			return null;
		}

		var $canvas = $(canvas).data('qrcode', qr), context = $canvas[0]
				.getContext('2d');
	...
	// Returns a `canvas` element representing the QR code for the given
	// settings.
	createCanvas = function(settings) {

		var $canvas = $('<canvas/>').attr('width', settings.size).attr(
				'height', settings.size);
		return drawOnCanvas($canvas, settings);
	},

	// Returns an `image` element representing the QR code for the given
	// settings.
	...
	createDiv = function(settings) {
		...
		var settings_size = settings.size, settings_bgColor = settings.background, math_floor = Math.floor,

		moduleCount = qr.moduleCount, moduleSize = math_floor(settings_size
				/ moduleCount), offset = math_floor(0.5 * (settings_size - moduleSize
				* moduleCount)),
	...
	var qrcode = function() {
		...
		var qrcode = function(typeNumber, errorCorrectLevel) {
			...
			var createBytes = function(buffer, rsBlocks) {
				...
				for (var r = 0; r < rsBlocks.length; r += 1) {
					...
					var rawPoly = qrPolynomial(dcdata[r],
							rsPoly.getLength() - 1);
			...
			var createData = function(typeNumber, errorCorrectLevel, dataList) {
				...
				if (buffer.getLengthInBits() > totalDataCount * 8) {
					throw new Error('code length overflow. ('
							+ buffer.getLengthInBits() + '>' + totalDataCount
							* 8 + ')');
				}
		...
		/**
		 * @param unicodeData
		 *            base64 string of byte array. [16bit Unicode],[16bit
		 *            Bytes], ...
		 * @param numChars
		 */
		...
		var QRUtil = function() {
			...
			var G18 = (1 << 12) | (1 << 11) | (1 << 10) | (1 << 9) | (1 << 8)
					| (1 << 5) | (1 << 2) | (1 << 0);
			var G15_MASK = (1 << 14) | (1 << 12) | (1 << 10) | (1 << 4)
					| (1 << 1);
			...
			_this.multiply = function(e) {
				...
				for (var i = 0; i < _this.getLength(); i += 1) {
					for (var j = 0; j < e.getLength(); j += 1) {
						num[i + j] ^= QRMath.gexp(QRMath.glog(_this.get(i))
								+ QRMath.glog(e.get(j)));
					}
				}
			...
			var bitOutputStream = function(out) {
				...
				_this.write = function(data, length) {
					...
					while (_bitLength + length >= 8) {
						_out
								.writeByte(0xff & ((data << _bitLength) | _bitBuffer));
						length -= (8 - _bitLength);
						data >>>= (8 - _bitLength);
						_bitBuffer = 0;
						_bitLength = 0;
